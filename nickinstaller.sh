        ...
        case $opciones in
                ...
                3) read -p "introduce la ruta absoluta para crear el directorio de configuracion para el contenedor de tvheadend:" dirtvheadend
                   ...
                   if [ $? -eq 0 ]; then
                         ...
                         sudo docker run -d --name=$nombretvheadend --net=host -e TZ=Europe/Madrid  -v $dirtvheadend:/config -v $dirtvheadendgrab:/recordings --restart unless-stopped lscr.io/linuxserver/tvheadend
                   else
                         echo "error"
                   fi
